...
$stmt = $conexion->prepare("SELECT Booking_BK, costoEXW, num_op FROM despacho WHERE DespachoID = ?");
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  btn.addEventListener('click', () => {
    ...
    fetch('../api/despacho/actualizarliquidaciondespacho.php', {
      ...
      body: JSON.stringify({ datos, coeficiente, idDespacho, invoices })
    })
    .then(r => r.json())
    .then(json => {
      if (json.success) {
                ...
                <div class="col-6">
                  <div class="d-grid">
                    <button class="preset-btn btn" data-value="true" onclick="layout_sidebar_change('dark');">
                      <span class="btn-label">Dark</span>
                  <div class="col-md-6">
                    <label class="form-label fw-bold">Mapped Invoices</label>
                    <div class="form-control bg-light"><?= $invoicesDisplay ?></div>
                      <span class="pc-lay-icon"><span></span><span></span><span></span><span></span></span>
                    </button>
                  </div>
                </div>
